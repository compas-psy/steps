//! Нативный мост к SQLite — ОБЩИЙ для оболочек Android и Windows
//! (ADR-0005).
//!
//! Изначально файл жил в `apps/mobile/src-tauri/src/sqlite.rs`. Вынесен в
//! отдельный крейт, когда десктопной оболочке понадобилась та же
//! персистентность: до этого Windows работала на `kind: 'memory'` и теряла
//! всё при перезапуске. Копировать 870 строк вместе с их тестами во вторую
//! оболочку значило бы завести две реализации одного контракта, которые
//! разойдутся на первом же исправлении. Ни строки платформенно-специфичного
//! кода здесь нет: путь к файлу даёт `app.path().app_data_dir()`, а он
//! кросс-платформенный.
//!
