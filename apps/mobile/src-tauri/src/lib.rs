...
//!
//! Команды `sqlite_*` — не исключение из этого правила, а ровно
//! платформенная возможность: доступ к файлу нативной SQLite, которого у
//! WebView нет. Ни схемы, ни SQL приложения там нет — только транспорт
//! (ADR-0005). Реализация переехала в общий крейт `shagi-sqlite`
//! (`crates/shagi-sqlite`), потому что ровно тот же мост понадобился
//! десктопной оболочке: до этого Windows работала на `kind: 'memory'` и
//! теряла всё при перезапуске. Поведение и тесты переехали без изменений.
