...
async function main() {
  ...
  // ЖИВО ЛИ ПРИЛОЖЕНИЕ — первым вопросом, и порядок здесь не косметический:
  // если «Назад» ушла в активность и закрыла продукт, WebView вместе с ним
  // мёртв, и любой следующий `Runtime.evaluate` не получит ответа — то есть
  // упрётся в таймаут CDP (30 с) на каждой попытке опроса вместо того, чтобы
  // назвать настоящую причину сразу.
  if (adbSoft(['shell', 'pidof', APPLICATION_ID]).trim() === '') {
    fail(
      'аппаратная «Назад» из карточки задачи закрыла приложение — системная кнопка уходит в ' +
        'активность вместо навигации внутри продукта',
    );
  }
