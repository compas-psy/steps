 ...
 * ── Где что живёт ──────────────────────────────────────────────────────────
 *
 * Управление устройством (`adb`, мост DevTools, снятие файла базы,
 * скриншоты, ожидающие клики) переехало в `./device.mjs` — тем же и по той
 * же причине, что выражения страницы живут в `./page-actions.mjs`: у него
 * появился второй потребитель, короткая приёмка `./android-acceptance.mjs`.
 * Здесь остались только СЦЕНАРИЙ и утверждения о продукте.
 */
...
/**
 * Доставка BOOT_COMPLETED только ресиверу плагина, без запуска UI (Step 6c).
 *
 * В области модуля, а не внутри `main()`: после переезда `adbSoft`/`fail`/
 * `APPLICATION_ID` в `./device.mjs` она перестала замыкать хоть что-нибудь
 * из тела `main()` и стала обычной чистой обёрткой над `adb`
 * (`unicorn/consistent-function-scoping` — тот же разбор, что у `pad2`
 * выше). Поведение шага не меняется: те же аргументы, та же проверка
 * «Broadcast completed», то же сообщение.
 */
...
async function main() {
  ...
  // Пояс сдвигается НА ЗАПАД, а не на восток, и это не косметика.
  //
  // Прогон `33943613771` сменил пояс по-настоящему (`+0000 → +0900`,
  // Asia/Tokyo) — и `dumpsys alarm` оказался пуст. Приложение при этом
  // ПРАВО: напоминание стоит на сегодняшние ~04:15 по локальному времени,
  // а после сдвига на +9 часы устройства показывают уже 13:1x того же дня,
  // то есть момент напоминания прошёл почти девять часов назад, и
  // планировать его нельзя (та самая overdue-политика, которую отдельно
  // проверяет Step 7). Восточный сдвиг сталкивает claim #9 с другим
  // правилом и проверяет не то.
  //
  // `America/Los_Angeles` (UTC−7 в сентябре) двигает локальные часы НАЗАД:
  // то же самое напоминание остаётся в будущем на несколько часов, а его
  // абсолютный instant обязан измениться — ровно предмет claim #9.
  // Строгость проверок не меняется: триггерное время обязано стать другим,
  // число будильников — прежним.
  const NEW_TIMEZONE = 'America/Los_Angeles';
