...
### Второй раунд патча: persisted-слой оживлён на пути ЗАПИСИ (мандат владельца)

Первая версия патча трогала только чтение (`getSavedNotifications()`) и
оставляла persisted-слой мёртвым: `get_pending` всегда пуст,
`LocalNotificationRestoreReceiver` не восстанавливает ничего. Владелец
квалифицировал это как критический MVP-gap — frozen-требование «после реальной
перезагрузки напоминания восстанавливаются через `BOOT_COMPLETED` БЕЗ открытия
ШАГОВ» так не выполняется. Вендоренный плагин и `[patch.crates-io]`
сохраняются; A6, SQLite, доменная модель, `NotificationSchedulerPort`,
детерминированные native id и cancel-before-batch не менялись.

- **Патч №1** (`appendNotifications()`) — сериализуется объект `Notification`,
  а не всегда-`null` `sourceJson`. Пишется **собственным** мапper'ом с
  видимостью по умолчанию: мапper плагина (`FIELD ANY`) выдаёт и backing-поля
  Kotlin, и bean-имена, а оба ресивера читают строгим `ObjectMapper()` и
  падают на `isGroupSummary` мимо своего `catch (JSONException)`. Измерено на
  Jackson `2.15.3` — той самой версии, что объявлена в
  `tauri-2.11.5/mobile/android/build.gradle.kts:43`; разбор — ADR-0008 и
  `vendor/.../PATCH.md`.
- **Патч №2** (`NotificationScheduleSerializer`) — пишется `allowWhileIdle`,
  который апстрим читал, но не сохранял: иначе восстановленный alarm молча
  деградировал бы с `setExactAndAllowWhileIdle(RTC_WAKEUP)` до `setExact(RTC)`.
- **Патч №3** — прежний defensive-фильтр остаётся: чистит legacy-записи
  `"null"` со сборок до патча №1.

**Step 6c** (новый) — исполняемая приёмка обеих функций: валидность записи в
`NOTIFICATION_STORE.xml` → сырой `get_pending` = ровно 1 запись с верными
id/title/`at.date`/`allowWhileIdle` → принудительное снятие alarm'ов (уцелевший
замаскировал бы неработающий restore) → `BOOT_COMPLETED` ресиверу БЕЗ запуска
UI (проверяется отсутствием DevTools-сокета; отказ доставки отличается от
неработающего restore отдельным fail'ом) → ровно 1 восстановленный alarm →
повторный broadcast без дубля.
