...
export const DATE_PATTERNS: readonly PatternDef<DateChipValue>[] = [
  ...
  },
  {
    ...
    // формами.
    //
    // Предлог "на"/"в" ВХОДИТ в диапазон чипа, если он есть, — тот же приём
    // и то же обоснование, что у времени "в 11" (`matchers/time.ts`,
    // заголовок). Без этого "Шашлыки на выходные" давало заголовок
    // «Шашлыки на», а "уточнить часы работы в выходные" — «уточнить часы
    // работы в»: висящий предлог оставался в названии задачи. Золотой
    // корпус закреплял это как ожидаемое (`weekend-01..05`) — ошибка самого
    // корпуса, исправлена вместе с грамматикой.
    regex: new RegExp(
      `${WORD_BOUNDARY_BEFORE}(?:(?:на|в)\\s+)?выходны(?:е|х)${WORD_BOUNDARY_AFTER}`,
      'uy',
    ),
