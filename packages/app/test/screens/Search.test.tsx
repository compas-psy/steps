 ...
 * тестам реконсиляции ниже нужно НАЧАТЬ с уже-запланированным id, чтобы
 * проверить именно `cancel`. */
...
interface Seed {
  ...
  readonly reminders?: readonly ReturnType<typeof makeExplicitReminder>[];
}
...
describe('Search — реконсиляция расписания напоминаний после завершения (00§7 шаг 5, Task A4)', () => {
  it('завершение задачи с активным напоминанием отменяет его в scheduler', async () => {
    const user = userEvent.setup();
    const task = makeTask({ title: 'Оплатить интернет с напоминанием' });
    ...
    renderSearch({ tasks: [task], reminders: [reminder] }, host);
    await user.type(await searchInput(), 'интернет с напоминанием');

    const box = await screen.findByRole('checkbox', { name: 'Оплатить интернет с напоминанием' });
    await user.click(box);
