 ...
 * 2. Запасной — «в хранилище есть хоть одна задача» (`Launch.tsx`).
 *    Нужен там, где флага нет и быть не может: `localPreferences`
 *    недоступен (`Unavailable`, SPEC §4 — порт вправе отсутствовать), либо
 *    установка сделана ДО появления этого флага. Без запасного сигнала
 *    такой человек получил бы онбординг поверх своих же задач.
 *
